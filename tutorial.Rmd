...
lambda.0 <- rep(5, K)
parms.0 <- HSMM.init(obs, K, lambda.0) # initial values
HSMM.MLE <- HSMM.mle(K, obs, parms.0)
cat("mllk:", HSMM.MLE$mllk, " AIC:", HSMM.MLE$AIC, " BIC:", HSMM.MLE$BIC, "\n")
```

```{r MLE_out, include=TRUE, echo=TRUE, eval=TRUE,cache=TRUE,results='hide'}
# psuedo residuals + most likely state sequence (viterbi)
HSMM.pseudo_residuals(obs, m, HSMM.MLE$lambda, 
                      HSMM.MLE$mu, HSMM.MLE$sigma, HSMM.MLE$gamma, plt = TRUE)
HSMM.viterbi(obs, m, HSMM.MLE$lambda, HSMM.MLE$mu, HSMM.MLE$sigma, HSMM.MLE$gamma, plt = TRUE)
```
...
data.stan <- list(N = length(obs), K = K, y = obs,
                  ...
                  a_0 = rep(8, K), b_0 = rep(1, K), alpha_0 = 1)
